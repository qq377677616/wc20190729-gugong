...
Page({
  ...
  quebtn() {
    if (this.data.is_fly) {
      wxb.jump_redirectTo("/pages/wish/wish?id=9")
    } else {
      wxb.jump_redirectTo("/pages/logs/logs?id=9")
    }
   ...
   */
  jump_btn() {
    wxb.Post('foodapi/temple/limi_pass', { openid: wxb.getStorages("openid"), zd: 9 }, res => {
      console.log(res)
      if (res.data.code == 0) {
        var is_fly = false
