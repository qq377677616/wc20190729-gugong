...
Page({
   ...
   */
  jump_btn() {
    wxb.Post('foodapi/temple/limi_pass', { openid: wxb.getStorages("openid"), zd: 20 }, res => {
      console.log(res)
      if (res.data.code == 0) {
        var is_fly = false
        ...
        //   exchange: false
        // })
        wxb.jump_redirectTo("/pages/logs/logs?id=20")
      }
    })
  ...
  quebtn() {
    if (this.data.is_fly) {
      wxb.jump_redirectTo("/pages/wish/wish?id=20")
    } else {
      wxb.jump_redirectTo("/pages/logs/logs?id=20")
    }
